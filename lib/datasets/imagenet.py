...
class imagenet(imdb):
    ...
    def _get_wnid(self,index):
        wnid = ''
        endwnid = index.find('_')
        if endwnid > 0:
            wnid = index[:endwnid]
        return wnid
    ...
    def image_path_from_index(self, index):
        ...
        image_path = os.path.join(self._data_path, 'Images',
                                self._get_wnid(index), index + self._image_ext)
    ...
    def _load_imagenet_annotation(self, index):
        ...
        filename = os.path.join(self._data_path, 'Annotations', self._get_wnid(index), index + '.xml')
